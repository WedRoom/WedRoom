...
public class FreeDAO {
	...
	public List<FreeDTO> getArticles(int start,int end) {
		...
		List<FreeDTO> articleList=null;//ArrayList articleList=null;//(O)
			...
			if(search==null || search=="") {
			   sql="select * from (select * from Free order by free_no desc) where rownum >= ? and rownum <= ?";
				...
				if(search.equals("all")) {
					sql="select * from (select * from Free where title like '%"+
				           searchtext+"%' or content like '%"+searchtext+"%' order by free_no desc) where rownum >= ? and rownum <= ?";
				}else {
				   sql="select * from (select * from Free where "+search+" like  '%"+
				           searchtext+"%' order by free_no desc) where rownum >= ? and rownum <= ?";
				}
			}
	...
	public void insertArticle(FreeDTO article) {
		...
		try {
			...
			sql="insert into Free(free_no,title,id,content,created_datetime) values(?,?,?,?,?)";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1,article.getFree_no());
			pstmt.setString(2, article.getTitle());
			pstmt.setString(3, article.getId());
			pstmt.setString(4, article.getContent());
			pstmt.setTimestamp(5, article.getCreated_datetime());
	...
	public FreeDTO getArticle(int free_no) {
		FreeDTO article=null;//ArrayList articleList=null;//여러개 담을때
		...
		try {
			...
			if(rs.next()) {//보여주는 결과가 있다면
				 article=makeArticleFromResult();//생성된 객체를 얻어온다.
				  /*
				  article=new FreeDTO();//MemberDTO=>필드별로 담을것.
				  article.setFree_no(rs.getInt("free_no"));
				  article.setId_no(rs.getInt("id_no"));
				  article.setTitle(rs.getString("title"));
				  article.setId(rs.getString("id"));
	...
	//2)수정시키는 메서드 작성
		public int updateArticle(FreeDTO article) {
			...
			try {
				...
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, article.getTitle());// 웹에서는 이미 데이터저장된 상태(Setter~)
...
//	public int updateArticle(FreeDTO article) {// insertArticle(BoardDTO article)
//
...
//		try {
//			con = pool.getConnection();
//			sql = "select id from Free where free_no=?";
//			pstmt = con.prepareStatement(sql);
//			pstmt.setInt(1, article.getFree_no());
//			rs = pstmt.executeQuery();
//			if (rs.next()) {// 보여주는 결과가 있다면
//				dbid = rs.getString("id");
//				System.out.println("dbid=>" + dbid);// 확인한 뒤에 나중에 삭제해야한다.
//				System.out.println("article.getId()=>"+article.getId());
//				if (dbid.equals(article.getId())) {
//					System.out.println("게시판의 글수정 아이디=>" + article.getId());
//					sql = "update Free set title=?,content=?,created_datetime=? where free_no=? and id=?";
//					pstmt = con.prepareStatement(sql);
//					pstmt.setString(1, article.getTitle());// 웹에서는 이미 데이터저장된 상태(Setter~)
//					pstmt.setString(2, article.getContent());
//					pstmt.setTimestamp(3, article.getCreated_datetime());
...
//	public int deleteArticle(int free_no,String id) { //회원탈퇴와 동일한 기능
//		
//		String dbid = null;// db에서 찾은 아이디를 저장
//		int x = -1;// 게시물의 삭제성공유무
//		
//		System.out.println("deleteArticle에서의 free_no=>"+free_no+",id=>"+id);
//
//		try {
//			con = pool.getConnection();
//			sql = "select passwd from Free where free_no=?";
//			pstmt = con.prepareStatement(sql);
//			pstmt.setInt(1, free_no);
//			rs = pstmt.executeQuery();
//			if (rs.next()) {// 보여주는 결과가 있다면
//				dbid = rs.getString("passwd");
//				System.out.println("dbid=>" + dbid);// 확인한 뒤에 나중에 삭제해야한다.
//				if (dbid.equals(id)) {
//					sql = "delete from Free where free_no=?";
//					pstmt = con.prepareStatement(sql);
