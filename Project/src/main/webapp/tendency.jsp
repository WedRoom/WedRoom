<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="login.*,member.*,java.io.PrintWriter"%>
...
<%
//int id_no=0;
//id_no=(Integer)session.getAttribute("id_no");
/* if(session.getAttribute("id_no") != null) {
	id_no=(Integer)session.getAttribute("id_no");
} */
int id_no=(int)session.getAttribute("id_no");
%>
        <!-- 성향테스트 -->
        <div class="back">
            <form method="post" id="tendency" name="tendency" onsubmit="return tendencyCheck()" action="/Project/tendencyPro.do?id_no=${id_no}">
...
function tendencyCheck(){
	if(document.tendency.sleepinghabbit.value==""){
		alert("잠버릇 유무를 체크해주세요.");
		document.tendency.sleepinghabbit.focus();
		return false;
	}
	if(document.tendency.smoking.value==""){
		alert("흡연 유무를 체크해주세요.");
		document.tendency.smoking.focus();
		return false;
	}
	if(document.tendency.pet.value==""){
		alert("반려동물 유무를 체크해주세요.");
		document.tendency.pet.focus();
		return false;
	}
