...
public class NoticeDAO{
	...
	public int getArticleCount() { 
		...
		try {
			...
			sql="select count(*) from Notice";//작성
	...
	public List<NoticeDTO> getArticles(int start,int end) {
		...
		try {
			con=pool.getConnection();
			//sql="select * from (select * from Notice order by free_no desc) where rownum >= ? and rownum <= ?";
			sql="SELECT *  FROM (SELECT ROWNUM RNUM, n.*  FROM (SELECT * FROM Notice order by notice_no desc) n) where rnum > ? and rnum <= ?";
			pstmt=con.prepareStatement(sql);
			...
			if(search==null || search=="") {
			   //sql="select * from (select * from Notice order by free_no desc) where rownum >= ? and rownum <= ?";
				...
				if(search.equals("all")) {
					sql="select * from (select rownum rnum,n.* from (select * from Notice where title like '%"+
							searchtext+"%' or content like '%"+searchtext+"%' order by notice_no desc) n) where rnum > ? and rnum <= ?";
				}else {
					sql="select * from (select rownum rnum,n.* from (select * from Notice where title like '%"+search+"%' or content like '%"+
							   searchtext+"%' order by notice_no desc) n) where rnum > ? and rnum <= ?";
				}
			}
	...
	public void insertArticle(NoticeDTO article) {
		int notice_no=article.getNotice_no();//0 신규글
		
		int number=0;//데이터를 저장하기위한 게시물번호 
		System.out.println("insertArticle메서드의 내부 notice_no=>"+notice_no);
		...
		try {
			...
			if(rs.next()) {//보여주는 결과가 있다면
				int max=rs.getInt(1);
				article.setNotice_no(max+1);
				//number=rs.getInt(1)+1;//최대값+1
				System.out.println("insertArticle메서드의 number=>"+number);
			}else {
				number=1;//테이블에 한개의 데이터가 없다면
			}
			sql="insert into Notice(notice_no,id_no,title,id,content,created_datetime,views) values(?,?,?,?,?,?,?)";
			pstmt=con.prepareStatement(sql);
			pstmt.setInt(1,article.getNotice_no());
			...
			System.out.println("notice_no"+article.getNotice_no());
	...
	public NoticeDTO getArticle(int notice_no) {
		...
		try {
			...
			sql="update Notice set views=views+1 where notice_no=?";
			pstmt=con.prepareStatement(sql);
			...
			if(rs.next()) {//보여주는 결과가 있다면
				 article=makeArticleFromResult();//생성된 객체를 얻어온다.
				  ...
				  article.setNotice_no(rs.getInt("free_no"));
				  article.setId_no(rs.getInt("id_no"));
				  article.setTitle(rs.getString("title"));
				  article.setId(rs.getString("id"));
				  article.setContent(rs.getString("content"));
	...
	public NoticeDTO updateGetArticle(int notice_no) {
	...
	//2)수정시키는 메서드 작성
		public int updateArticle(NoticeDTO article) {
			...
			try {
				con = pool.getConnection();
				sql = "update Notice set title=?,content=?,created_datetime=? where notice_no=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, article.getTitle());
				pstmt.setString(2, article.getContent());
				pstmt.setTimestamp(3, article.getCreated_datetime());
				pstmt.setInt(4, article.getNotice_no());
				int update = pstmt.executeUpdate();
