...
//				pstmt.setInt(2, end);//불러와서 담을 갯수(ex 10)
	...
	// -----------------------------------------------------------------
		...
		public void insertArticle(MateDTO article, TendencyDTO ten) {
			...
			try {
				...
				sql="insert into friend(mate_no,id_no,gender,title,lifestyle,other_matter,views,filename,fileRealname,created_datetime) values(?,?,?,?,?,?,?,?,?,sysdate)";
				pstmt=con.prepareStatement(sql);
	...
	public MateDTO getArticle(int num) {
		...
		try {
			...
			sql = "select a.mate_no, a.id_no, a.title, a.gender, to_char(a.created_datetime,'yyyy-mm-dd') as created_datetime, "
